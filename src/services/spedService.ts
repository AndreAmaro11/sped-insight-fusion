...
import { SpedRecord, SpedProcessedData } from '@/types/sped';
import { saveSpedDataToDatabase } from './database';
import { toast } from 'react-toastify';

export async function parseSpedFile(fileContent: string, fileName: string, userId: string): Promise<SpedProcessedData> {
  ...
  for (let line of lines) {
    ...
    if (recordType === 'J100' || recordType === 'J150') {
      const accountCode = fields[2] || '';
      const accountDescription = fields[3] || '';
      const finalBalance = fields[6] || '0';
      const debitAmount = fields[4] || '0';
      const creditAmount = fields[5] || '0';
      ...
      records.push({
        ...
        debitAmount,
        creditAmount,
        blockType: recordType,
        fiscalYear,
      });
    }
  }

  // Buscar o ID da empresa com base no CNPJ
  if (cnpj) {
    ...
    if (companyError) {
      console.error("Erro ao buscar empresa pelo CNPJ:", companyError.message);
    } else if (companies) {
      companyId = companies.id;
      console.log("Empresa encontrada:", companyId);
    } else {
      console.warn(`Nenhuma empresa encontrada com o CNPJ ${cleanCnpj}`);
    }
  }

  if (records.length === 0) {
    console.error("Não foi possível extrair nenhum registro válido.");
  ...
  await saveSpedDataToDatabase(processedData, fileName, fileContent.length, userId, companyId);
